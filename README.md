...
| 窗口位置 | 退出时记住窗口位置/大小与最大化状态，下次启动原样恢复（限制在最近显示器工作区内，防拔掉显示器后落到屏幕外）；首次运行居中显示 |
